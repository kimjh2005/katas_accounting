...
using useraccount_table = eosio::multi_index<"useraccount"_n, useraccount, eosio::indexed_by<"by.user"_n, eosio::const_mem_fun<useraccount, uint64_t, &useraccount::get_user_value>>>;
...
class accounting : public eosio::contract {
  private:
	...
	std::tuple< bool, const category * > find_category(const eosio::name &user_name, const eosio::name &category_name)
	{
        ...
        for ( auto itr = category_user_index.lower_bound(user_name.value); itr != category_user_index.upper_bound(user_name.value); ++itr ) {
  ...
  public:
    ...
    accounting( eosio::name receiver, eosio::name code, eosio::datastream<const char*> ds ):contract(receiver, code, ds),
    ...
    void createuser( eosio::name user_name, eosio::asset initial_asset ) {
        ...
        eosio::check( !eosio::is_account(user_name), "account " + user_name.to_string() + " already exists" );
        ...
        if( auto [exists, ptr] = find_category(user_name, default_category); !exists ) {
         	...
         	eosio::check(false, "found an orphan category " + default_category.to_string() + " " + user_name.to_string());
         }
      }
    ...
    void print( eosio::name user_name ) {
        ...
        const auto &category_user_index = _category_table.get_index<"by.user"_n>();
        for ( auto itr = category_user_index.lower_bound(user_name.value); itr != category_user_index.upper_bound(user_name.value); itr++ ) {
      	   output += ",[" + std::to_string(itr->id) + "|" + itr->category_name.to_string() + "|" + std::to_string(itr->balance.amount) + "]";
        }
